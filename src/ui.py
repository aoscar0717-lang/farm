...
TIME_SCALE_LABELS = {0.0: "已暫停", 1.0: "1x", 2.0: "2x", 4.0: "4x"}
TIME_SCALE_BADGE_COLORS = {
    0.0: ((150, 40, 40), (255, 120, 120)),   # paused: red
    1.0: ((60, 60, 60), (200, 200, 200)),    # normal: neutral grey
    2.0: ((60, 90, 150), (140, 190, 255)),   # sped up: blue
    4.0: ((150, 100, 30), (255, 190, 90)),   # sped up more: amber
}


def _draw_time_scale_hud(screen, state):
    """Small always-visible badge (current speed / PAUSED) plus, only
    while actually paused, a large unmissable center banner -- reads
    state["time_scale"] and nothing else; every position/size here comes
    from ui_layout so this function has no geometry of its own to drift
    out of sync with anything."""
    time_scale = state.get("time_scale", 1.0)
    label = TIME_SCALE_LABELS.get(time_scale, f"{time_scale:g}x")
    bg_color, border_color = TIME_SCALE_BADGE_COLORS.get(time_scale, ((60, 60, 60), (200, 200, 200)))

    badge = ui_layout.time_scale_badge_rect()
    pygame.draw.rect(screen, bg_color, badge, border_radius=8)
    pygame.draw.rect(screen, border_color, badge, 2, border_radius=8)
    txt = font_tiny.render(f"{label}  (P/[/])", True, WHITE)
    screen.blit(txt, (badge.centerx - txt.get_width() // 2, badge.centery - txt.get_height() // 2))

    if time_scale == 0:
        cx, cy = ui_layout.paused_banner_center()
        banner_text = font_large.render("已暫停 PAUSED", True, (255, 220, 220))
        pad_x, pad_y = 24, 12
        panel = pygame.Surface((banner_text.get_width() + pad_x * 2, banner_text.get_height() + pad_y * 2), pygame.SRCALPHA)
        panel.fill((40, 10, 10, 200))
        pygame.draw.rect(panel, (255, 120, 120, 220), panel.get_rect(), 2, border_radius=10)
        panel.blit(banner_text, (pad_x, pad_y))
        screen.blit(panel, (cx - panel.get_width() // 2, cy - panel.get_height() // 2))
...
def draw_hud(screen, state, current_tool, active_zone, mouse_pos=(-1, -1)):
    ...
    _draw_time_scale_hud(screen, state)
