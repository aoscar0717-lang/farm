...
def _mouse_over_ui_chrome(ctx):
    ...
    rects = (
        ...
        _ui_layout.time_scale_badge_rect(),
    )
...
def _hovered_dog(state, ctx):
    """Which dog (as its (index, meta) pair) the hover cell is near, or
    None. Distinct from _hovering_entity("dogs") -- that only answers
    "is *a* dog nearby" (bool); this resolves *which* one, so the Hover
    text can read that specific dog's dog_meta mode (see
    capstone_contract.py's _process_zone_dogs / _day_tick_farm_dogs, which
    are what actually write mode/target into dog_meta each tick)."""
    pos = ctx.get("hover_pos")
    if pos is None:
        return None
    zone = state[ctx["active_zone"]]
    dogs = zone.get("dogs", [])
    meta = zone.get("dog_meta", [])
    for i, (dx, dy) in enumerate(dogs):
        if math.hypot(pos[0] - dx, pos[1] - dy) <= 15:
            m = meta[i] if i < len(meta) else {"mode": "idle", "target": None}
            return (i, m)
    return None


def _dog_status_text(state, ctx):
    """Section 二 of the pet-automation request: different text per the
    dog's real current mode, read from dog_meta (not re-derived here) so
    this can never say something that isn't actually true of that dog's
    AI state this tick."""
    hovered = _hovered_dog(state, ctx)
    if hovered is None:
        return "這隻狗會主動攻擊靠近的敵人。"
    _, m = hovered
    mode = m.get("mode", "idle")
    target = m.get("target")
    if mode == "fighting":
        return "狗狗正在與入侵者搏鬥！"
    if mode == "chasing_enemy":
        return "狗狗正在追擊入侵者！"
    if mode == "chasing_crop":
        crop_type = state["farm"]["crop_data"].get(target, {}).get("type") if target else None
        name = _crop_display_name(crop_type) if crop_type else "作物"
        return f"狗狗正在幫忙收割{name}！"
    if mode == "cooling_down":
        return "狗狗剛收割完一株作物，正在稍作休息。"
    # mode == "idle" (or anything unrecognized -- fail toward the safest,
    # always-true description rather than claiming an action that isn't
    # happening).
    if state.get("phase") == "day":
        return "狗狗正在呼呼大睡，等作物成熟就會自動幫忙收割。"
    return "狗狗正在呼呼大睡，沒有察覺到威脅。"
...
THOUGHT_ENTRIES = [
    ...
    {
        "id": "info_dog_nearby",
        "text": _dog_status_text,
        "priority": 31,
        "trigger": "hover",
        "condition": lambda state, ctx: _hovering_entity(state, ctx, "dogs", radius=15),
        "detail_key": lambda state, ctx: (_hovered_dog(state, ctx) or (None,))[0],
    },
    {
    ...
    },
    {
        ...
        "text": lambda state, ctx: (
            "遊戲目前已暫停，按 P 恢復。" if state.get("time_scale", 1.0) == 0
            else f"目前遊戲速度為 {state.get('time_scale', 1.0):g} 倍速。按 P 暫停，[ 和 ] 可以調整速度。"
        ),
        # 33, not 34 -- more specific than ui_stats_row (34), whose hover
        # rect (top_panel_stats_row_rect) is deliberately generous/nearly
        # full-width and genuinely overlaps the time-scale badge's real
        # position below the money readout. Lower number = wins the tie
        # structurally, same pattern as ui_tutorial_sidebar_task (33) vs
        # ui_tutorial_sidebar (35) above.
        "priority": 33,
        "trigger": "hover",
        "condition": lambda state, ctx: (
            not ctx["shop_open"] and _hovering_rect(ctx, _ui_layout.time_scale_badge_rect())
        ),
    },
    {
