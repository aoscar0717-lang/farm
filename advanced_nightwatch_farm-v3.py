    ...
    def _init_ui(self):
        ...
        self.cards_by_tab = {
            "CROPS": [
                ...
                # 建造成本（0G+30工藝+3個精鐵錠+10個木料），跟金幣/
                # 科技點數兩張卡片「$金額」的呈現風格做出區分，比照
                # SPRINKLER 卡片「2錠」這種標示物品成本的既有寫法。
                # 【後續修正：燒過的礦物】使用者糾正風車要的物料是「燒
                # 過的礦物」，即熔爐煉出的精鐵錠 (metal_ingot)，不是未
                # 加工的礦石結晶 (metal_ore)，文案同步從「礦石結晶」
                # 改成「精鐵錠」，跟 build_cost_items 的真實 key 一致。
                ("PLACE_FARM_WINDMILL", "農業風車", "30工藝+3精鐵錠+10木料 | 需種在農田上，手持種子點擊可3x3範圍播種，並每3秒自動收成", "farm_windmill"),
            ],
            "DECO": [
